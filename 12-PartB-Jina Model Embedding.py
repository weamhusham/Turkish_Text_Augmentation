...
model_path = r"D:\Important\WeamCollectiveLearning\Project\Models\jinaai_jina-embeddings-v3"
...
def generate_embeddings(texts, transformer_model, tokenizer):
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(texts), batch_size), desc="Generating embeddings"):
            ...
            batch_embeddings = (
                outputs.pooler_output.to(dtype=torch.float32).cpu().numpy()
                if hasattr(outputs, "pooler_output") and outputs.pooler_output is not None
                else outputs.last_hidden_state[:, 0, :].to(dtype=torch.float32).cpu().numpy()
            )
...
def process_datasets(datasets_group, tokenizer, transformer_model, test_dataset):
    ...
    # Ensure the test file path is a valid string and file exists
    ...
    for dataset in datasets_group:
        ...
        # Ensure the train file path is a valid string and file exists
...
def save_results_table(results, results_path):
    ...
    output_file = os.path.join(results_path, "training_augmentation_results_Jina_Embeddings_v3.csv")
